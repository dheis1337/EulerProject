...
# First four entries in the first row
pairs[1, 1:4]

# First four entries in the first column
pairs[1:4, 1]

for (i in 1:(ncol(pairs) - 4)) {
  products <- c(products, prod(c(pairs[i + 1, i], pairs[i + 2, i + 1], 
                                 pairs[i + 3, i + 2], pairs[i + 4, i + 3])))
}

transpairs <- t(pairs)
for (i in 1:(ncol(pairs) - 4)) {
  products <- c(products, prod(c(transpairs[i + 1, i], transpairs[i + 2, i + 1], 
                                 transpairs[i + 3, i + 2], transpairs[i + 3, i + 3])))
}
